...
const ADDITIVE_COLUMNS = [
  ...
  // Equipment status changes may now originate on a handset (dok 05 §3), so the
  // history row needs the same idempotency key and attribution the field tables
  // carry. Nullable on purpose: rows written by an admin have no client_id, and
  // the UNIQUE index over this column ignores NULLs.
  ['equipment_status_log', 'client_id', 'TEXT', null],
  ['equipment_status_log', 'shift_group', "TEXT DEFAULT ''", null],
  ['equipment_status_log', 'shift_time', "TEXT DEFAULT ''", null],
  ['equipment_status_log', 'received_at', 'TEXT', "datetime('now')"],
];
