...
/**
 * Structural caps on a lembar tugas (doc 05 §4).
 *
 * Not arbitrary: 12 columns is already more than fits on a handset screen one
 * row at a time, and 200 rows is a full plant walk. Past either number the
 * lembar has stopped being a round and become a spreadsheet, which is a
 * different product. Enforced at the admin boundary, where the supervisor can
 * be told, rather than at sync where the operator would pay for it.
 */
export const SHEET_LIMITS = { columns: 12, rows: 200 };

/**
 * Column kinds (doc 05 §4).
 *
 * Stored as free text rather than a CHECK constraint so VIDEO can be added
 * later without a migration on databases already in the field. The enum here is
 * the real gate.
 *
 *   LABEL  — read-only text the supervisor wrote; carries no cell
 *   TEXT   — free text
 *   NUMBER — numeric, `options` holds the unit shown beside the field
 *   PHOTO  — one photo; `example_photo` shows the operator the target framing
 *   CHECK  — done / not done
 *   CHOICE — one of `options`, a JSON array of strings
 */
export const SHEET_COLUMN_KINDS = ['LABEL', 'TEXT', 'NUMBER', 'PHOTO', 'CHECK', 'CHOICE'];
export const SHEET_STATUSES = ['OPEN', 'DONE', 'CANCELLED'];
...
/**
 * A row an operator added in the field to a lembar the supervisor had already
 * published (doc 05 §4).
 *
 * `sortOrder` is absent on purpose: an appended row goes to the end, and the
 * server decides where that is. A handset offline for two days has no idea how
 * many rows the sheet grew in the meantime, so any position it proposed would
 * be a guess that reorders someone else's work.
 */
export const sheetRowSchema = z.object({
  clientId,
  sheetId: z.number().int().positive(),
  label: z.string().min(1).max(CAPS.rowLabel),
  ...attribution,
});
 ...
 *
 * The row is addressed either by `rowId` (a row the phone pulled from the
 * server) or by `rowClientId` (a row the same batch is creating). Both are
 * optional here and the pair is checked below, because "neither was sent" is a
 * client bug worth naming precisely rather than a missing-field error on
 * whichever one zod happened to check first.
 *
 * Cells are append-only, so nothing here identifies a cell to overwrite: the
 * latest `filledAt` for a (row, column) is the value that counts.
 */
export const sheetCellSchema = z.object({
  clientId,
  sheetId: z.number().int().positive(),
  rowId: z.number().int().positive().nullable().optional().default(null),
  rowClientId: z.string().regex(UUID_V4).nullable().optional().default(null),
  columnId: z.number().int().positive(),
  valueText: z.string().max(CAPS.cellText).optional().default(''),
  valueNumber: z.number().finite().nullable().optional().default(null),
  photoPath,
  filledAt: clientTimestamp,
  ...attribution,
}).refine(
  (cell) => cell.rowId !== null || cell.rowClientId !== null,
  { message: 'Sel harus menyebut rowId atau rowClientId', path: ['rowId'] },
);
...
export const syncSchema = z.object({
  ...
  // A full 200-row lembar with 5 fillable columns is 1000 cells, so the cap
  // here is higher than the others. The phone still batches; this is the
  // ceiling on one request, not on a sync.
  sheetRows: z.array(sheetRowSchema).max(500).optional().default([]),
  sheetCells: z.array(sheetCellSchema).max(2000).optional().default([]),
});
...
/**
 * `options` is stored as a JSON array string but accepted as an array, so the
 * route never has to trust hand-written JSON from the browser. For NUMBER
 * columns a single-element array carries the unit ("mm", "bar").
 */
export const sheetColumnSchema = z.object({
  label: z.string().min(1).max(CAPS.columnLabel),
  kind: z.enum(SHEET_COLUMN_KINDS).optional().default('TEXT'),
  isRequired: z.boolean().optional().default(true),
  options: z.array(z.string().min(1).max(60)).max(12).optional().default([]),
  examplePhoto: z.string().max(200).regex(PHOTO_PATH).or(z.literal('')).optional().default(''),
  hint: z.string().max(CAPS.hint).optional().default(''),
  sortOrder: z.number().int().min(0).max(999).optional().default(0),
  isActive: z.boolean().optional().default(true),
});

export const sheetRowAdminSchema = z.object({
  label: z.string().min(1).max(CAPS.rowLabel),
  sortOrder: z.number().int().min(0).max(999).optional().default(0),
  isActive: z.boolean().optional().default(true),
});
...
/**
 * Creating a lembar. Columns and rows may arrive with it — the builder submits
 * the whole design in one request so a half-built sheet never reaches a
 * handset, which is the failure the operator would notice.
 */
export const sheetSchema = z.object({
  title: z.string().min(1).max(CAPS.sheetTitle),
  description: z.string().max(CAPS.description).optional().default(''),
  status: z.enum(SHEET_STATUSES).optional().default('OPEN'),
  dueDate: z.string().max(40).nullable().optional().default(null),
  // '' means every shift sees it. Matched against shift_accounts.display_name,
  // the same string sync stamps onto records (lib/sync.js header).
  assignedShift: z.string().max(20).optional().default(''),
  allowOperatorRows: z.boolean().optional().default(true),
  isActive: z.boolean().optional().default(true),
  columns: z.array(sheetColumnSchema).max(SHEET_LIMITS.columns).optional().default([]),
  rows: z.array(sheetRowAdminSchema).max(SHEET_LIMITS.rows).optional().default([]),
});

/** A cell filled from the admin grid. Same shape as the field one, minus sync. */
export const sheetCellAdminSchema = z.object({
  rowId: z.number().int().positive(),
  columnId: z.number().int().positive(),
  valueText: z.string().max(CAPS.cellText).optional().default(''),
  valueNumber: z.number().finite().nullable().optional().default(null),
  photoPath: z.string().max(200).regex(PHOTO_PATH).or(z.literal('')).optional().default(''),
});
