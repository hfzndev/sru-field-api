...
import {
  equipmentSelect, serializeEquipmentRow, sheetBundle, sheetCellsFor, tankDeviation,
...
export function buildPull(db, account, since) {
  ...
  return {
    dataVersion,
    // The lembar design travels as master (delta, soft-deletable); the filled
    // cells travel as recent, because they are field records like everything
    // else there — see sheetCellsFor() for why they are not shift-scoped.
    master: {
      ...masterData,
      ...sheetBundle(db, account.displayName, (alias) => deltaClause(since, alias), deltaArgs(since)),
    },
    recent: {
      ...recent(db, account.displayName),
      sheetCells: sheetCellsFor(db, account.displayName, RECENT_LIMIT),
    },
