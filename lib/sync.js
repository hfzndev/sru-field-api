 ...
 */
function handleEquipmentStatus(db, record, out) {
  ...
  `).run(
    ...
    record.operatorName, record.shiftGroup, record.shiftTime, record.changedAt,
  );
...
export function processSync(db, payload) {
  ...
  db.transaction(() => {
    ...
    for (const record of payload.equipmentStatus) handleEquipmentStatus(db, record, out);
  })();
