...
/**
 * Reads one image out of a multipart request and stores it.
 *
 * Shared by the device route (POST /api/upload) and the admin one
 * (POST /api/admin/upload), which differ only in the credential they accept.
 * The checks below are the security boundary (doc 08 §7) and two hand-written
 * copies of them is how one of the pair ends up subtly weaker — the same
 * argument lib/admin/photo makes for sharing resolveStoredPhoto.
 *
 * @returns {{ok: true, path: string}|{ok: false, response: Response}}
 */
export async function readUploadedImage(request) {
  ...
  if (declared > MAX_UPLOAD_BYTES + MULTIPART_OVERHEAD) {
    return { ok: false, response: errorResponse(413, 'PAYLOAD_TOO_LARGE', 'Foto terlalu besar (maksimal 5MB)') };
  }
  ...
  try {
    ...
    return { ok: false, response: errorResponse(400, 'VALIDATION_ERROR', 'Format upload tidak valid') };
  }

  const file = form.get('file');
  if (!file || typeof file.arrayBuffer !== 'function') {
    return { ok: false, response: errorResponse(400, 'VALIDATION_ERROR', 'Field "file" wajib diisi') };
  }
  ...
  if (!detected) {
    return { ok: false, response: errorResponse(415, 'UNSUPPORTED_MEDIA_TYPE', 'File harus berupa foto JPEG, PNG, atau WebP') };
  }

  return { ok: true, path: storeUpload(buffer, detected.ext) };
}
