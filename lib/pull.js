    ...
    SELECT e.id, e.tag_number, e.name, e.unit_key, e.location, e.status, e.is_active,
      ...
      LEFT JOIN equipment_status_log l
        ON l.id = (SELECT id FROM equipment_status_log
                    ...
                    -- Ordered by received_at, NOT changed_at. changed_at holds
                    -- two formats -- ISO from a handset, SQLite's space-
                    -- separated form from the admin panel's column default --
                    -- and SQLite compares them as text, so 'T' (0x54) beats
                    -- ' ' (0x20) at index 10 and any phone entry outranks any
                    -- admin entry on the same date whatever the real time.
                    -- received_at is written by this server on every row and
                    -- is uniform. changed_at is still what gets displayed: it
                    -- is when the operator saw the fault, which is the honest
                    -- thing to show. It just must not decide the ordering.
                    -- id breaks the tie: both columns have one-second resolution.
                    ORDER BY received_at DESC, id DESC LIMIT 1)
     WHERE ${deltaClause(since, 'e')} ORDER BY e.id
  `).all(...args).map((e) => ({
    id: e.id,
