...
/**
 * Login must hand back the same shape delta pull does, note included: a phone
 * that logs in fresh and one that pulls a delta must end up showing the same
 * screen (doc 06 §4).
 */
function activeEquipment(db) {
  return db.prepare(`
    SELECT e.id, e.tag_number, e.name, e.status, e.status_changed_at,
           l.description AS status_note, l.changed_by_name AS status_changed_by
      FROM equipment e
      LEFT JOIN equipment_status_log l
        ON l.id = (SELECT id FROM equipment_status_log
                    WHERE equipment_id = e.id
                    -- received_at, not changed_at -- see lib/pull.js for why.
                    ORDER BY received_at DESC, id DESC LIMIT 1)
     WHERE e.is_active = 1 ORDER BY e.tag_number
  `).all().map((e) => ({
    id: e.id,
